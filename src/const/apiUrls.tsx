export const API_URL = import.meta.env.VITE_SERVER_API;
export const API_SHOWS_URL = `${API_URL}/shows/actualShows`;
export const API_SHOWS_CREATE_URL = `${API_URL}/shows/`;
export const API_SHOWS_DELETE_URL = `${API_URL}/shows/deleteShow`;
export const API_SHOWS_UPDATE_URL = `${API_URL}/shows/updateShow`;
export const API_SECTIONS_URL = `${API_URL}/secciones/padre/`;
export const API_SPOTS_URL = `${API_URL}/lugares/by-seccion-padre/`;
export const API_SPOTS_CREATE_URL = `${API_URL}/lugares/`;
export const API_SPOTS_UPDATE_URL = `${API_URL}/lugares/`;
export const API_SPOTS_DELETE_URL = `${API_URL}/lugares/`;
export const API_COMERCIOS_URL = `${API_URL}/lugares/`;
export const API_COMERCIOS_CREATE_URL = `${API_URL}/lugares/`;
export const API_COMERCIOS_UPDATE_URL = `${API_URL}/lugares/`;
export const API_COMERCIOS_DELETE_URL = `${API_URL}/lugares/`;
export const API_COLORS = `${API_URL}/colors`;
export const API_SETTINGS = `${API_URL}/settings`;
export const API_BRANDING = `${API_URL}/branding`;
export const API_BANNERS_URL = `${API_URL}/banners`;
export const API_BANNERS_CREATE_URL = `${API_URL}/banners`;
export const API_BANNERS_UPDATE_URL = `${API_URL}/banners`;
export const API_BANNERS_DELETE_URL = `${API_URL}/banners`;

